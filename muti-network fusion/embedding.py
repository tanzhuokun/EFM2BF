...
from data_process import netsDataset, load_networks
...
def train_epoch(dataloader, model, idx_layer, criterion,  optimizer, epoch):
    ...
    for step, (X, y, indx) in enumerate(dataloader):
        optimizer.zero_grad()
        time.sleep(0.01)#防止RuntimeError: CUDA error: an illegal memory access was encountered
...
def run(args):
    ...
    for idx_layer in range(num_layers):
        ...
        for idx_net in range(cfg["net_nums"]):
            ...
            #     保存loss
            global train_loss
            np.savez(str(idx_net+1) + ".npz", loss=train_loss)
            print(train_loss)
            train_loss = []
