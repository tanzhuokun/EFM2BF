...
class autoEncoder(nn.Module):
    def __init__(self, input_dim, hidden):
        ...
        self.linears_decoder = nn.ModuleList(
            [nn.Linear(self.input_hidden[i], self.input_hidden[i-1]) for i in range(len(self.input_hidden)-1, 0, -1)][::-1]#[::-1] ### 取从后向前（相反）的元素
        )
...
class encoderLoss(nn.Module):
    def __init__(self, mustlinks, batch_size,
        ...
        self._gamma = gamma
        self.mse = torch.nn.MSELoss(reduction='sum') #返回 loss.sum()为标量
